...
Release `v2.3.0` publishes:

```text
world-capabilities-v2.3.0-deterministic.tar.gz
world-capabilities-v2.3.0-deterministic.tar.gz.sha256
```

The stable release URL is:

```text
https://github.com/tkersey/world-capabilities/releases/download/v2.3.0/world-capabilities-v2.3.0-deterministic.tar.gz
```
...
```sh
(cd .. && shasum -a 256 -c world-capabilities-v2.3.0-deterministic.tar.gz.sha256)
bun conformance/check-distribution.mjs --root .
sh conformance/run-conformance.sh \
  --archive ../world-capabilities-v2.3.0-deterministic.tar.gz \
  --checksum ../world-capabilities-v2.3.0-deterministic.tar.gz.sha256
```
