...
export const SLOT_PATHS = Object.freeze({
  readme: "README.md",
...
export const WRITABLE_SLOTS = Object.freeze([
  "methods_source", "errors_source", "router_source", "index_source"
]);
const DOCUMENT_ROLE_NAMES = DOCUMENT_SLOTS;
...
export function decodeReadRequest(bytes) {
  return decode(bytes, "ERR_ADEQUACY_READ_REQUEST", (reader) => Object.freeze({
    slot: reader.enumName(DOCUMENT_SLOTS, "slot"),
...
export function decodeReplaceRequest(bytes) {
  return decode(bytes, "ERR_ADEQUACY_REPLACE_REQUEST", (reader) => Object.freeze({
    slot: reader.enumName(DOCUMENT_SLOTS, "slot"),
    path: reader.text(LIMITS.path, "path"),
  ...
  return decode(bytes, "ERR_ACTUALITY_ACTION", (reader) => {
    ...
    switch (action) {
      ...
      case "read_file": argumentsValue = Object.freeze({
        slot: reader.enumName(DOCUMENT_SLOTS, "arguments.slot"),
...
export function encodeReadResult(value) {
  const writer = new Writer("ERR_ADEQUACY_READ_RESULT");
  encodeSnapshotInto(writer, value, "result");
  return writer.finish();
}
...
export function encodeTestResult(value) {
  const writer = new Writer("ERR_ADEQUACY_TEST_RESULT");
  const [exitCode, passed, output, truncated] = exactRecord(
    value,
    ["exitCode", "passed", "output", "truncated"],
    writer.errorCode,
    "result"
  );
  writer.i32Value(exitCode, "exitCode");
  writer.bool(passed, "passed");
  writer.text(output, LIMITS.process, "output");
  writer.bool(truncated, "truncated");
  return writer.finish();
}
...
export function encodeReplaceOutcome(value) {
  const [kind, payload] = exactRecord(
    value,
    ["kind", "payload"],
    "ERR_ADEQUACY_REPLACE_OUTCOME",
    "result"
  );
  const writer = new Writer("ERR_ADEQUACY_REPLACE_OUTCOME");
  writer.enumValue(kind, REPLACE_OUTCOME_NAMES, "kind");
  switch (kind) {
    case "applied": {
      const [slot, slotCode, path, oldSha256, newSha256, alreadyApplied, current] = exactRecord(
        payload,
        ["slot", "slotCode", "path", "oldSha256", "newSha256", "alreadyApplied", "current"],
        writer.errorCode,
        "payload"
      );
      encodeSlotAndCode(writer, slot, slotCode, "payload");
      ...
      encodeSnapshotInto(writer, current, "payload.current");
      break;
    }
    case "denied": {
      const [slot, path, reason] = exactRecord(payload, ["slot", "path", "reason"], writer.errorCode, "payload");
      writer.enumValue(slot, DOCUMENT_SLOTS, "payload.slot");
      writer.text(path, LIMITS.path, "payload.path");
      writer.text(reason, LIMITS.denialReason, "payload.reason");
      break;
    }
    case "conflict": {
      const [slot, slotCode, path, expectedSha256, actualSha256] = exactRecord(
        payload,
        ["slot", "slotCode", "path", "expectedSha256", "actualSha256"],
        writer.errorCode,
        "payload"
      );
      encodeSlotAndCode(writer, slot, slotCode, "payload");
...
function decodeDecisionViewFrom(reader) {
  return Object.freeze({
    ...
    documents: reader.vector(LIMITS.documents, "context.documents", (_, index) =>
      decodeReadResultFrom(reader, `context.documents[${index}]`)),
    latestSearch: decodeOptional(reader, "context.latest_search", () => decodeSearchResultFrom(reader, "context.latest_search")),
    latestTest: decodeOptional(reader, "context.latest_test", () => decodeTestResultFrom(reader, "context.latest_test")),
    latestReplace: decodeOptional(reader, "context.latest_replace", () => decodeReplaceOutcomeFrom(reader, "context.latest_replace")),
    mutations: reader.vector(LIMITS.mutations, "context.mutations", (_, index) =>
      decodeMutationFrom(reader, `context.mutations[${index}]`)),
    evidence: Object.freeze({
      baselineFailureObserved: reader.bool("context.evidence.baseline_failure_observed"),
      latestTestPassed: reader.bool("context.evidence.latest_test_passed"),
      mutationCount: reader.u32(),
      lastTestMutationCount: reader.u32(),
      testCount: reader.u32()
    })
  });
}
...
function decodeReadResultFrom(reader, label) {
  const slot = reader.enumName(DOCUMENT_SLOTS, `${label}.slot`);
  const slotCode = reader.fixed(1)[0];
  if (slotCode !== DOCUMENT_SLOTS.indexOf(slot)) actualityFail(reader.errorCode, `${label}.slot_code`);
  return Object.freeze({
    slot,
    slotCode,
...
function decodeTestResultFrom(reader, label) {
  return Object.freeze({
    ...
    output: reader.text(LIMITS.process, `${label}.output`),
    truncated: reader.bool(`${label}.truncated`)
  });
}
...
function decodeReplaceOutcomeFrom(reader, label) {
  ...
  if (kind === "applied") {
    ...
    return Object.freeze({ kind, payload: Object.freeze({
      ...
      current: decodeReadResultFrom(reader, `${label}.current`)
    }) });
  }
  if (kind === "denied") return Object.freeze({ kind, payload: Object.freeze({
    slot: reader.enumName(DOCUMENT_SLOTS, `${label}.slot`),
    path: reader.text(LIMITS.path, `${label}.path`),
...
function decodeMutationFrom(reader, label) {
  const slot = reader.enumName(DOCUMENT_SLOTS, `${label}.slot`);
  const slotCode = reader.fixed(1)[0];
  if (slotCode !== DOCUMENT_SLOTS.indexOf(slot)) actualityFail(reader.errorCode, `${label}.slot_code`);
  return Object.freeze({
...
function encodeReadRequestInto(writer, value) {
  const [slot, path] = exactRecord(value, ["slot", "path"], writer.errorCode, "arguments");
  writer.enumValue(slot, DOCUMENT_SLOTS, "arguments.slot");
  if (SLOT_PATHS[slot] !== path) actualityFail(writer.errorCode, "arguments.path");
...
function encodeReplaceRequestInto(writer, value) {
  const [slot, path, expectedSha256, replacement, rationale] = exactRecord(
    value,
  ...
  writer.enumValue(slot, DOCUMENT_SLOTS, "arguments.slot");
  if (!WRITABLE_SLOTS.includes(slot) || SLOT_PATHS[slot] !== path) actualityFail(writer.errorCode, "arguments.path");
  writer.text(path, LIMITS.path, "arguments.path");
...
function encodeFinalResultInto(writer, value) {
  const [summary, changedFiles, testsPassed, mutationCount] = exactRecord(
    value,
  ...
  writer.u32Value(mutationCount, "arguments.mutation_count");
}
...
function decodeFinalResultFrom(reader) {
  return Object.freeze({
    ...
    mutation_count: reader.u32()
  });
}

function encodeSlotAndCode(writer, slot, slotCode, label) {
  writer.enumValue(slot, DOCUMENT_SLOTS, `${label}.slot`);
  if (!Number.isInteger(slotCode) || slotCode !== DOCUMENT_SLOTS.indexOf(slot)) {
    actualityFail(writer.errorCode, `${label}.slotCode`);
  }
  writer.fixed(Buffer.from([slotCode]));
}

function encodeSnapshotInto(writer, value, label) {
  const [slot, slotCode, path, sha256, contents] = exactRecord(
    value,
    ["slot", "slotCode", "path", "sha256", "contents"],
    writer.errorCode,
    label
  );
  encodeSlotAndCode(writer, slot, slotCode, label);
  if (SLOT_PATHS[slot] !== path) actualityFail(writer.errorCode, `${label}.path`);
  writer.text(path, LIMITS.path, `${label}.path`);
  writer.digestText(sha256, `${label}.sha256`);
  writer.text(contents, LIMITS.file, `${label}.contents`);
}
