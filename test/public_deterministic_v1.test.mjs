...
test("v2.3.1 deterministic distribution binds the reviewed ENF runtime bytes", async () => {
  expect(await runtimeTreeDigest(process.cwd())).toEqual({
    fileCount: 120,
    sha256: "01f9112d91b28d8476a84e08e39b7f17ac4838bd0789fb5a41f650f5738055c8",
  });
});
...
test("deterministic distribution is reproducible and safely self-verifying", async () => {
  ...
  try {
    ...
    expect(repositoryReadme).toContain("(cd .. && shasum -a 256 -c world-capabilities-v2.3.1-deterministic.tar.gz.sha256)");
